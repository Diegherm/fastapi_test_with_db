...
from inventory.crud import check_password, insert_user, select_user, select_users, update_user
from inventory.database import get_db
from inventory.models import User

from inventory.schemas import UserCreateSchema, LoginSchema, UserReadSchema, UserUpdateSchema
...
async def create_user(user: UserCreateSchema, session: Session = Depends(get_db)) -> User:
...
async def patch_user(user_id: int, user_data: UserUpdateSchema, session: Session = Depends(get_db)) -> User:
